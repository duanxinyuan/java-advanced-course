...
- [<font color=red>数据库</font>](#font-colorred数据库font)
  - [<font color=blue>ACID</font>](#font-colorblueacidfont)
  - [<font color=blue>索引技术</font>](#font-colorblue索引技术font)
      - [B 树](#b-树)
          - [**B 树的查找流程：**](#b-树的查找流程)
      - [**B 树和平衡二叉树的不同点：**](#b-树和平衡二叉树的不同点)
      - [B+树](#b树)
      - [B+Tree VS B-Tree](#btree-vs-b-tree)
      - [LSM-Tree](#lsm-tree)
          - [**LSM-Tree 写入数据**](#lsm-tree-写入数据)
          - [**LSM-Tree 读取数据**](#lsm-tree-读取数据)
  ...
  - [<font color=blue>MySQL</font>](#font-colorbluemysqlfont)
      - [存储引擎（MyISAM、InnoDB）](#存储引擎myisaminnodb)
      - [脏读、不可重复读、幻读](#脏读不可重复读幻读)
      - [事务隔离级别（读未提交、读已提交、可重复读、串行化）](#事务隔离级别读未提交读已提交可重复读串行化)
      - [故障恢复](#故障恢复)
      - [MySQL 架构](#mysql-架构)
      - [MySQL 的文件](#mysql-的文件)
      - [binlog](#binlog)
      - [Innodb 如何保证事务的原子性、一致性、隔离性、持久性？](#innodb-如何保证事务的原子性一致性隔离性持久性)
      - [数据写入流程](#数据写入流程)
      - [redo log 的两段式提交](#redo-log-的两段式提交)
      - [根据主键数据读取流程](#根据主键数据读取流程)
      - [MySQL 索引](#mysql-索引)
      - [MySQL 中的锁](#mysql-中的锁)
      - [explain](#explain)
      - [分库分表](#分库分表)
      ...
      - [读写分离](#读写分离)
      - [主从复制](#主从复制)
      - [MySQL 小知识点](#mysql-小知识点)
  - [<font color=blue>ShardingSphere</font>](#font-colorblueshardingspherefont)
  - [<font color=blue>Redis</font>](#font-colorblueredisfont)
      - [数据库结构](#数据库结构)
      - [底层数据结构](#底层数据结构)
      - [数据类型](#数据类型)
      - [过期策略（定期删除+惰性删除）](#过期策略定期删除惰性删除)
      - [内存淘汰策略：noeviction、allkeys-lru、allkeys-random、volatile-lru、volatile-random、volatile-ttl](#内存淘汰策略noevictionallkeys-lruallkeys-randomvolatile-lruvolatile-randomvolatile-ttl)
      - [Redis 主从复制方式（三种）](#redis-主从复制方式三种)
      - [部署模式：单机、主从、哨兵、分片、集群](#部署模式单机主从哨兵分片集群)
      - [持久化（RDB、AOF）](#持久化rdbaof)
      - [缓存雪崩、缓存穿透、缓存击穿](#缓存雪崩缓存穿透缓存击穿)
      - [Redis 事务](#redis-事务)
      - [Redis 分布式锁（setnx）](#redis-分布式锁setnx)
      - [为什么 Redis 是单线程的](#为什么-redis-是单线程的)
      - [Redis IO 多路复用](#redis-io-多路复用)
      - [为什么 Redis 选择使用跳表而不是红黑树来实现有序集合？](#为什么-redis-选择使用跳表而不是红黑树来实现有序集合)
  - [<font color=blue>MongoDB</font>](#font-colorbluemongodbfont)
      - [存储引擎](#存储引擎)
          - [MMAPv1 引擎](#mmapv1-引擎)
          - [WiredTiger 引擎](#wiredtiger-引擎)
      - [索引](#索引)
      - [集群方式](#集群方式)
  - [<font color=blue>ES</font>](#font-colorblueesfont)
      ...
      - [读写流程](#读写流程)
          - [根据条件搜索数据](#根据条件搜索数据)
          - [根据 doc id 读取数据](#根据-doc-id-读取数据)
          - [写入流程](#写入流程)
          - [shard 写入数据（增删改）的原理](#shard-写入数据增删改的原理)
      - [倒排索引的原理](#倒排索引的原理)
      - [查询性能优化](#查询性能优化)
