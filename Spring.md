...
- [<font color=red>Spring</font>](#font-colorredspringfont)
  - [<font color=blue>SpringMVC</font>](#font-colorbluespringmvcfont)
      - [SpringMVC 组件说明](#springmvc-组件说明)
      - [SpringMVC 工作流程](#springmvc-工作流程)
  - [<font color=blue>SpringBoot</font>](#font-colorbluespringbootfont)
      - [SpringBoot 启动流程](#springboot-启动流程)
      - [Spring 常用注解](#spring-常用注解)
      - [Spring 中 Bean 的作用域与生命周期](#spring-中-bean-的作用域与生命周期)
      - [Spring IOC（Inversion of Control）原理](#spring-iocinversion-of-control原理)
      - [Spring IOC 容器的初始化过程](#spring-ioc-容器的初始化过程)
      - [Spring AOP（Aspect Orient Programming）原理及应用场景](#spring-aopaspect-orient-programming原理及应用场景)
      - [Spring AOP 和 AspectJ AOP 的区别](#spring-aop-和-aspectj-aop-的区别)
      - [Spring 中如何将接口注册成 Bean](#spring-中如何将接口注册成-bean)
      - [BeanFactory 和 ApplicationContext 有什么区别？](#beanfactory-和-applicationcontext-有什么区别)
      - [构造方法注入和 set 注入有什么区别？](#构造方法注入和-set-注入有什么区别)
      - [SpringBoot 相对于 Spring 做了哪些改进？](#springboot-相对于-spring-做了哪些改进)
      - [Spring 5 比 Spring4 做了哪些改进？](#spring-5-比-spring4-做了哪些改进)
      - [Spring 事务原理、隔离级别、传递机制](#spring-事务原理隔离级别传递机制)
      - [Spring Batch（任务批处理）](#spring-batch任务批处理)
  - [<font color=blue>SpringCloud</font>](#font-colorbluespringcloudfont)
      - [Config 配置中心](#config-配置中心)
      - [Bus 事件总线](#bus-事件总线)
      - [Eureka 服务注册与发现](#eureka-服务注册与发现)
      - [Consul、Zookeeper 服务注册与发现](#consulzookeeper-服务注册与发现)
      - [Feign 网络远程调用](#feign-网络远程调用)
      - [Zuul、Gateway 网关](#zuulgateway-网关)
      - [Ribbon 负载均衡](#ribbon-负载均衡)
      - [Hystrix（缓存、流控、熔断、降级、统计、监控、告警）](#hystrix缓存流控熔断降级统计监控告警)
          - [Hystrix 线程机制实现资源控制(Thread)](#hystrix-线程机制实现资源控制thread)
          - [Hystrix 信号量机制实现资源控制(Semaphore)](#hystrix-信号量机制实现资源控制semaphore)
          - [Hystrix 细粒度资源控制](#hystrix-细粒度资源控制)
          - [Hystrix 的 fallback（降级）机制](#hystrix-的-fallback降级机制)
          - [Hystrix 断路器](#hystrix-断路器)
      - [Task 分布式任务调度](#task-分布式任务调度)
  - [<font color=blue>SpringCloud Alibaba</font>](#font-colorbluespringcloud-alibabafont)
      - [Nacos 服务注册与发现、配置中心、网络远程调用、负载均衡](#nacos-服务注册与发现配置中心网络远程调用负载均衡)
      - [Sentinel 限流](#sentinel-限流)
      - [Seata 分布式事务](#seata-分布式事务)
      - [SchedulerX 分布式任务调度](#schedulerx-分布式任务调度)
      - [RocketMQ 消息总线](#rocketmq-消息总线)
